...
public class Osoba extends Table{
    ...
    public static Osoba login (String korisnickoIme, String lozinka, String uloga) throws Exception {
        String sql = "SELECT id FROM osoba WHERE korisnickoIme=? AND lozinka=? AND uloga=?";
        PreparedStatement query = Database.CONNECTION.prepareStatement(sql);
        query.setString(1, korisnickoIme);
        query.setString(2, lozinka);
        query.setString(3, uloga);
